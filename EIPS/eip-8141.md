...
- `max_fee_per_gas` -- the maximum [EIP-1559](./eip-1559.md) fee the transaction is willing to pay, per gas.
...
- `blob_versioned_hashes` -- list of [EIP-4844](./eip-4844.md) blob versioned hashes.
...
1. Execute a call with the specified `mode`, `flags`, `target`, `gas_limit`, `value`, and `data`.
    ...
    - Evaluate `registered_static_approval(frame, resolved_target, tx, validation_results)` before resolving or loading target code.
        - If it returns a scope, apply the native approval effects described above and skip all account-code resolution and execution for this frame, including [EIP-7702](./eip-7702.md) delegated code.
...
After execution, return `payer_refund` to the payer. This is the `resolved_target` that produced payment approval by calling `APPROVE(APPROVE_PAYMENT)` or `APPROVE(APPROVE_EXECUTION_AND_PAYMENT)`, or through the native registered-static equivalent. Return `max_gas - gas_used` to the block gas pool.

##### Blob handling

When `blob_versioned_hashes` is non-empty, the transaction is a blob-carrying transaction and follows [EIP-4844](./eip-4844.md), where the payer is also the blob-fee payer. The following distinctions from [EIP-4844](./eip-4844.md) must also be adhered to:
...
- Unlike [EIP-4844](./eip-4844.md) transactions, a frame transaction is not required to carry blobs.
- The `BLOBHASH` instruction returns `tx.blob_versioned_hashes[index]` in every frame, per [EIP-4844](./eip-4844.md).
...
- execution performs `CALL*` or `EXTCODE*` to an address that is neither an existing contract nor a precompile, or to an address that uses an [EIP-7702](./eip-7702.md) delegation, except for `tx.sender` default-code behavior
- if a `deploy` frame is present, its execution does not result in non-empty code being installed at `tx.sender` (either conventional contract code or an [EIP-7702](./eip-7702.md) delegation indicator)
...
`EIP8130_ACTOR` has no transaction-selected registry and no implementation-configured authenticator allowlist. Consensus fixes the only storage address and the two accepted native authenticator selectors. Consensus validation records one exact storage key for every entry before frame processing.
...
The [EIP-7702](./eip-7702.md) authorization list heavily relies on ECDSA cryptography to determine the authority of accounts to delegate code. While delegations could be used in other manners later, it does not satisfy the PQ goals of the frame transaction.
...
`EIP8130_ACTOR` lets any account, including a deployed or [EIP-7702](./eip-7702.md) delegated smart account, opt into an EIP-8130 registry-backed root authorization path. Selecting scheme `0x03` at the deterministic signature index tells clients to validate the registered K1 or P256 actor natively and apply the requested VERIFY approval scope without executing wallet code. The account's registration is the source of truth for whether that actor and authenticator are authorized.
...
| ----- | ------ | ----------- | ----------------------------- | ------------- | ----- | -------------------- |
| 0     | VERIFY | ENTRY_POINT | APPROVE_EXECUTION_AND_PAYMENT | Null (sender) | 0     | Empty                |
| 1     | SENDER | Sender      | ATOMIC_BATCH_FLAG             | [ERC-20](./eip-20.md)        | 0     | approve(DEX, amount) |
| 2     | SENDER | Sender      | APPROVE_SCOPE_NONE            | DEX           | 0     | swap(...)            |

Frame 0 uses a signature entry with empty `msg` and calls `APPROVE(APPROVE_EXECUTION_AND_PAYMENT)`. Frames 1 and 2 form an atomic batch: if the swap in frame 2 reverts, the [ERC-20](./eip-20.md) approval from frame 1 is also reverted, preventing the account from being left with a dangling approval.
...
| 2     | SENDER  | Sender      | APPROVE_SCOPE_NONE | [ERC-20](./eip-20.md)        | 0     | transfer(Sponsor,fees) |
...
- Frame 1: Inspects signature metadata in `tx.signatures` if needed, and checks that the next frame is an [ERC-20](./eip-20.md) send of the right size to the sponsor. Calls `APPROVE(APPROVE_PAYMENT)` to authorize payment.
...
Note: to be included in the public mempool under the current model, sponsors must accept some risk of frontrunning by the sponsee who can zero out their [ERC-20](./eip-20.md) balance before the sponsored transaction lands on-chain.
...
This is not much larger than an [EIP-1559](./eip-1559.md) transaction; the extra overhead is mainly the need to specify the sender and the per-frame wrapper explicitly.
...
**Trustless pay-with-[ERC-20](./eip-20.md) sponsor (add these frames):**
...
Notes: Sponsor can read info from other fields. [ERC-20](./eip-20.md) transfer call is 68 bytes.

There is some inefficiency in the sponsor case, because the same sponsor address must appear in three places (sponsor validation, send to sponsor inside [ERC-20](./eip-20.md) calldata, post op frame), and the ABI is inefficient (~12 + 24 bytes wasted on zeroes). This is difficult to mitigate in a "clean" way, because one of the duplicates is inside the [ERC-20](./eip-20.md) call, "opaque" to the protocol. However, it is much less inefficient than [ERC-4337](./eip-4337.md), because not all of the data takes the hit of the 32-byte-per-field ABI overhead.
...
It's recommended that to *validate* the transaction, a specific frame structure is enforced and the amount of gas that is expended executing the validation phase must be limited. Once the validation prefix reaches payer approval via `APPROVE(APPROVE_PAYMENT)`, `APPROVE(APPROVE_EXECUTION_AND_PAYMENT)`, or the native registered-static equivalent, the transaction can be included in the mempool and propagated to peers safely.
...
In general, it can be assumed that handling of frame transactions imposes similar restrictions as [EIP-7702](./eip-7702.md) on mempool relay, i.e. only a single transaction can be pending for an account that uses frame transactions.
