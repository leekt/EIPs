...
- `signer` -- scheme-dependent signer metadata; for `SECP256K1`, `P256`, and `KEY_REGISTRY_MODE`, this is a 20-byte address.
...
for sig in tx.signatures:
    if sig.scheme in [SECP256K1, P256, KEY_REGISTRY_MODE]:
...
| 0x03..0xfe | reserved     | reserved                                       | reserved |
| 0xff       | `KEY_REGISTRY_MODE` | signature defined by the registered scheme | fixed SLOAD cost + underlying key type cost |

`KEY_REGISTRY_MODE` is defined as `0xff`. `KEY_REGISTRY_SLOAD_COST` is defined as `6300`, equivalent to three cold `SLOAD` charges.

For `SECP256K1` and `P256`, the `signer` is a 20-byte Ethereum address. If absent, `tx.sender` is used (for introspections as well).
For `KEY_REGISTRY_MODE`, the `signer` is the account whose registered key is used. If absent, `tx.sender` is used. Its `msg` MUST be empty.
...
A protocol-validated signature scheme may define a registered-key verification rule over `(publicKey, signature, msg)`. For `SECP256K1`, `publicKey` is the 20-byte address and `signature` retains the encoding above. For `P256`, `publicKey` is `qx || qy` and `signature` is `r || s`. A scheme without such a rule cannot be returned by the registry.

##### Registered Root Key Registry

`REGISTERED_KEY_REGISTRY` is a general-purpose key registry with the following interface:

```solidity
function setKey(bytes1 keyType, bytes calldata publicKey) external;

function getRegisteredKey(address account) public view returns(bytes1, bytes memory);
```

`setKey` replaces the key type and public key registered to the caller. `getRegisteredKey` returns the registered values for `account`. The registry stores `publicKey` as opaque bytes: it does not interpret key types, define public-key encodings, or perform signature verification. This EIP does not define the registry's storage layout or any key-type-specific registry methods.

When validating `KEY_REGISTRY_MODE`, the protocol calls `getRegisteredKey(resolved_signer)` and validates `sig.signature` using the scheme and public key returned by the registry. A failed call, malformed return value, unsupported scheme, invalid public key, or invalid signature makes the signature invalid. `ARBITRARY` and `KEY_REGISTRY_MODE` cannot be returned as the registered scheme, so registry resolution is never recursive.
...
def validate_signature(sig, tx_sender, sig_hash) -> bool:
    ...
    elif sig.scheme == KEY_REGISTRY_MODE:
        if sig.msg != Bytes():
            return False
        registered_scheme, public_key = REGISTERED_KEY_REGISTRY.getRegisteredKey(resolved_signer)
        return validate_registered_key_signature(registered_scheme, public_key, sig.signature, msg)
...
`validate_registered_key_signature` applies the registered-key verification rule defined by `registered_scheme`. It returns `false` when the scheme has no such rule, including for `ARBITRARY`, `KEY_REGISTRY_MODE`, and reserved or unsupported schemes.
...
def signature_gas(sig):
    ...
    if sig.scheme == P256:
        return 6700
    if sig.scheme == KEY_REGISTRY_MODE:
        return KEY_REGISTRY_SLOAD_COST + registered_scheme_gas(sig)
...
For `KEY_REGISTRY_MODE`, `registered_scheme_gas(sig)` is the verification cost of the scheme returned by `getRegisteredKey` while validating `sig`. It does not perform another registry lookup.
...
- If `mode` is `VERIFY`:
  ...
  - If there is not a signature `sig` at index `sig_index` with `sig.scheme` in `[SECP256K1, KEY_REGISTRY_MODE]` such that `resolved_signer == resolved_target` and `sig.msg == Bytes()`, revert.
