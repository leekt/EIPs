...
category: Core
created: 2026-01-29
requires: 1559, 2718, 3529, 3607, 4844, 7594, 7623, 7702, 7951, 8130
---
...
| `EIP8130_ACTOR`            | `0x3`             |
| `ACCOUNT_CONFIG_ADDRESS`   | Canonical address defined by [EIP-8130](./eip-8130.md) |
| `K1_AUTHENTICATOR`         | `address(0x01)`   |
| `P256_AUTHENTICATOR`       | Canonical P256 authenticator address defined by [EIP-8130](./eip-8130.md) |
| `ACTOR_STORAGE_COLD_COST`  | `2100`            |
| `ACTOR_STORAGE_WARM_COST`  | `100`             |
| `EIP8130_DEFAULT_EOA_REVOKED` | `0x01`         |
| `EIP8130_SCOPE_SENDER`     | `0x01`            |
| `EIP8130_SCOPE_POLICY`     | `0x02`            |
| `EIP8130_SCOPE_SELF_PAYER` | `0x08`            |
| `EIP8130_SCOPE_SPONSOR_PAYER` | `0x10`         |

`ACCOUNT_CONFIG_ADDRESS`, `P256_AUTHENTICATOR`, and the storage-slot derivation and packed-field decoding functions used below are imported normatively from EIP-8130. A fork MUST NOT activate `EIP8130_ACTOR` unless EIP-8130 has fixed each of them. They are chain-wide consensus constants and MUST NOT be implementation-configured or selected by the transaction.
...
For `EIP8130_ACTOR` entries, the authenticator address is carried inside the `signature` field, so the signature entry shape is unchanged. The Account Configuration Contract is implicitly `ACCOUNT_CONFIG_ADDRESS`.
...
- `signer` -- scheme-dependent signer metadata; for `SECP256K1` and `P256`, this is a 20-byte address; for `EIP8130_ACTOR`, this is the 20-byte account whose actor configuration is used.
- `msg` -- either empty, indicating the canonical transaction signature hash, or an explicit 32-byte digest.
- `signature` -- raw signature bytes interpreted according to `scheme`; for `EIP8130_ACTOR` entries, `authenticator (20 bytes) || authenticator-specific data`.
...
for sig in tx.signatures:
    ...
    elif sig.scheme == EIP8130_ACTOR:
        assert len(sig.signer) == 0 or len(sig.signer) == 20
        authenticator = sig.signature[0:20]
        data = sig.signature[20:]
        if authenticator == K1_AUTHENTICATOR:
            assert len(data) == 65  # r || s || v
        elif authenticator == P256_AUTHENTICATOR:
            assert len(data) == 128  # r || s || qx || qy
        else:
            invalid_transaction()
...
| 0x3        | `EIP8130_ACTOR`   | `authenticator (20 bytes) || data`, see [EIP-8130 Actor Signature Scheme](#eip-8130-actor-signature-scheme) | exact-slot access + 2800 or 6700 |
...
For `EIP8130_ACTOR`, the `signer` is the 20-byte account whose configured actor authenticates the signature. If absent, `tx.sender` is used. The authenticator address is the first 20 bytes of `signature`; the registry is always `ACCOUNT_CONFIG_ADDRESS` and is not encoded.
...
##### EIP-8130 Actor Signature Scheme

`EIP8130_ACTOR` (also referred to as `REGISTERED_STATIC`) is a narrow protocol fast path for an actor configured under [EIP-8130](./eip-8130.md). Signature scheme `0x03` is the dispatch signal for this path; no additional `VERIFY` flag is introduced. It does not define a generic registry or execute authenticator contracts. The only registry is the canonical Account Configuration Contract at `ACCOUNT_CONFIG_ADDRESS`, and the only accepted authenticators are `K1_AUTHENTICATOR` and the enshrined `P256_AUTHENTICATOR`.

An entry keeps the ordinary four-element signature shape:

```text
[EIP8130_ACTOR, signer, msg, authenticator || data]
```

The two accepted encodings and their complete consensus semantics are:

| Authenticator | `data` | `actor_id` | Verification cost |
|---|---|---|---|
| `K1_AUTHENTICATOR` | `r \|\| s \|\| v` (65 bytes) | `bytes32(bytes20(recovered_address))` | 2800 |
| `P256_AUTHENTICATOR` | `r \|\| s \|\| qx \|\| qy` (32 bytes each) | `keccak256(qx \|\| qy)` | 6700 |

`P256VERIFY(msg, r, s, qx, qy)` uses the verification and failure semantics of [EIP-7951](./eip-7951.md) for the corresponding 160-byte input. The costs above are fixed outer-signature verification costs: in particular, they do not inherit EIP-7951's EVM precompile gas cost or the gas schedule of any authenticator contract. The configuration-slot access cost is added separately.

Both authenticators are executed natively with the validation rules below; no `STATICCALL` occurs. Every other authenticator, including EIP-8130 passkey and delegate authenticators and all wallet-defined authenticators, is invalid under this scheme. Such authentication remains available through an `ARBITRARY` signature consumed by deployed or delegated account code in a `VERIFY` frame, subject to that frame's gas limit and the existing public-mempool trace rules. This fast path does not create a trace exception for EVM calls to `ACCOUNT_CONFIG_ADDRESS`. Adding another authenticator to this fast path requires a separate consensus change that fixes its byte encoding, validation semantics, and gas cost.

After native authentication, the protocol reads exactly one EIP-8130 configuration slot at `ACCOUNT_CONFIG_ADDRESS`. It applies EIP-8130's native secp256k1 self-actor rule when applicable; every other actor uses `actor_config(resolved_signer, actor_id)`. Validation returns and caches `actor_id`, `scope`, `expiry`, and the exact 32-byte storage slot read. This proves only that the actor is registered and live; it does not grant approval by itself. A `VERIFY` frame can consume the cached result through the registered-static path defined below. That path applies the cached scope to the requested approval context without loading or executing the resolved target's code. The slot is used consistently for gas accounting and mempool revalidation.

The raw `signature` bytes and cached actor-validation fields are not introspectable by EVM code. The authenticator selector is nevertheless committed to by the canonical signature hash.
...
The canonical signature hash is computed from a copy of the transaction. For a signature with empty `msg`, witness bytes that would create a circular commitment are elided. An `EIP8130_ACTOR` authenticator selector is not witness data: it selects consensus validation semantics and therefore remains in the hash. Only the bytes after that selector are elided.
...
`compute_sig_hash` MUST NOT mutate `tx`. Consequently, for empty-message `EIP8130_ACTOR` entries, changing only `data` does not change the hash, while changing `authenticator` does.
...
class SignatureValidation:
    actor: Optional[ActorValidation] = None

def validate_signature(sig, tx_sender, sig_hash) -> Optional[SignatureValidation]:
    if len(sig.msg) == 0:
        msg = sig_hash
    elif len(sig.msg) == 32:
        if sig.msg == b"\x00" * 32:
    ...
    if sig.scheme == SECP256K1:
        ...
        if authenticator == K1_AUTHENTICATOR:
            ...
            if recovered == address(0):
                return None
            actor_id = bytes32(bytes20(recovered))
        elif authenticator == P256_AUTHENTICATOR:
            if len(data) != 128:
                return None
            r = int.from_bytes(data[0:32], "big")
        ...
        if authenticator == K1_AUTHENTICATOR and actor_id == bytes32(bytes20(resolved_signer)):
            storage_slot = account_state_slot(resolved_signer)
            word = SLOAD(ACCOUNT_CONFIG_ADDRESS, storage_slot)
            account_state = decode_eip8130_account_state(word)
            if account_state.reserved != 0:
                return None
            if (account_state.flags & EIP8130_DEFAULT_EOA_REVOKED) != 0:
                return None
            scope = account_state.default_eoa_scope
            expiry = account_state.default_eoa_expiry
        else:
            storage_slot = actor_config_slot(resolved_signer, actor_id)
            word = SLOAD(ACCOUNT_CONFIG_ADDRESS, storage_slot)
            config = decode_eip8130_actor_config(word)
            if word == 0 or config.reserved != 0:
                return None
        ...
        return SignatureValidation(
            actor=ActorValidation(actor_id, scope, expiry, storage_slot)
        )
...
For `EIP8130_ACTOR`, `bytes32(bytes20(recovered))` means the 20 address bytes followed by 12 zero bytes, matching Solidity's conversion between fixed-size byte arrays. The configuration read is performed against the transaction's pre-execution state. Its result is immutable for the remainder of the transaction even if a later frame changes the same slot.

The `ActorValidation` value is client-internal metadata. It is cached by signature index for registered-static authorization, gas accounting, stateless witnesses, and mempool dependency tracking; it is not exposed through `SIGPARAM`. The validation SLOAD warms exactly `(ACCOUNT_CONFIG_ADDRESS, storage_slot)` in the transaction-wide accessed-state journal. It does not touch the authenticator address as an account and does not execute Account Configuration Contract or authenticator code.
...
1. In signature-list order, validate each `sig` with `validate_signature(sig, tx.sender, sig_hash)`. If it returns `None`, the transaction is invalid. Otherwise cache the immutable `SignatureValidation` value at the signature's index. For `EIP8130_ACTOR`, also record whether its exact configuration slot was cold or warm immediately before the validation SLOAD.
...
1. Process the frame with the specified `mode`, `flags`, `target`, `gas_limit`, `value`, and `data`. Registered-static `VERIFY` frames are evaluated directly; every other frame executes a call.
    ...
    - Before resolving or loading `resolved_target` code, check whether the frame is a [registered-static VERIFY frame](#registered-static-verify-frames).
        - If it is, evaluate it according to that subsection, append its frame receipt, and continue with the next frame.
        - The registered-static path is protocol-defined and does not perform an EVM call, an account-code lookup, or [EIP-7702](./eip-7702.md) delegation resolution for `resolved_target`.
        - If the selected signature has scheme `EIP8130_ACTOR` but any registered-static requirement fails, the frame reverts and the transaction is invalid; implementations MUST NOT fall back to executing account code.
    - Set frame's `caller`:
        - If mode is `SENDER`:
...
1. If frame has mode `VERIFY` and was not handled through registered-static verification, the following additional requirements are imposed:
...
##### Registered-static VERIFY frames

A `VERIFY` frame is **registered-static** when the signature at the deterministic index selected by its requested approval scope has scheme `EIP8130_ACTOR`.
...
assert allowed_scope != APPROVE_SCOPE_NONE

sig_index = 0 if allowed_scope & APPROVE_EXECUTION != 0 else 1
assert sig_index < len(tx.signatures)
sig = tx.signatures[sig_index]
assert sig.scheme == EIP8130_ACTOR
assert sig.msg == Bytes()
assert resolved_signer(sig) == resolved_target

result = validation_results[sig_index].actor
assert result is not None
scope = result.scope

assert (scope & EIP8130_SCOPE_POLICY) == 0
if allowed_scope & APPROVE_EXECUTION:
    assert scope == 0 or (scope & EIP8130_SCOPE_SENDER) != 0
if allowed_scope & APPROVE_PAYMENT and resolved_target == tx.sender:
    assert scope == 0 or (scope & EIP8130_SCOPE_SELF_PAYER) != 0
if allowed_scope & APPROVE_PAYMENT and resolved_target != tx.sender:
    assert scope == 0 or (scope & EIP8130_SCOPE_SPONSOR_PAYER) != 0

apply_approve(resolved_target, allowed_scope)
```

`apply_approve` performs exactly the checks and transaction-context effects specified for `APPROVE(allowed_scope)`, as though code executing with `ADDRESS == resolved_target` had invoked the instruction. It does not execute EVM code. The account's EIP-8130 registration is the explicit opt-in to this root-authorization path.

The frame succeeds with empty return data and logs, and reports `gas_used = 0`; its intrinsic per-frame cost, signature-verification cost, and EIP-8130 configuration-slot access cost are charged elsewhere as specified. Any unused `frame.gas_limit` is refunded normally. The path applies whether `resolved_target` has empty code, deployed code, or an EIP-7702 delegation indicator.
...
```python
def authenticator_of(sig):
    # EIP8130_ACTOR entries only.
    return sig.signature[0:20]

def signature_gas(sig, validation_result, actor_slot_was_warm):
    if sig.scheme == SECP256K1:
        return 2800
    if sig.scheme == P256:
        return 6700
    if sig.scheme == EIP8130_ACTOR:
        assert validation_result.actor is not None
        access_cost = (
            ACTOR_STORAGE_WARM_COST
            if actor_slot_was_warm
            else ACTOR_STORAGE_COLD_COST
        )
        if authenticator_of(sig) == K1_AUTHENTICATOR:
            auth_cost = 2800
        elif authenticator_of(sig) == P256_AUTHENTICATOR:
            auth_cost = 6700
        else:
            invalid_transaction()
        return access_cost + auth_cost
...
signature_verification_cost = sum(
    signature_gas(sig, validation_results[i], actor_slot_was_warm.get(i, False))
    for i, sig in enumerate(tx.signatures)
)
...
`actor_slot_was_warm[i]` is relevant only to `EIP8130_ACTOR` and records the access status of `(ACCOUNT_CONFIG_ADDRESS, validation_results[i].actor.storage_slot)` immediately before that signature's validation SLOAD. Signatures are validated in list order. Thus the first access to an exact slot costs `2100`, a later access to that same slot costs `100`, and two actors of the same account still pay two cold accesses when their exact slots differ. The validation access remains warm for subsequent frames.
...
- If `mode` is `VERIFY`:
  ...
  - Require `sig.scheme == SECP256K1`. `EIP8130_ACTOR` signatures are consumed before code dispatch by the registered-static path and therefore never reach default code.
  - Call `APPROVE(allowed_scope)`.
- If `mode` is `SENDER` or `DEFAULT`:
  - Return successfully as if calling empty code.

The empty-`msg` requirement is unconditional for default code: an explicit digest does not commit to the frame list and can never authorize execution or payment here. Accounts that need policy execution or an unsupported authenticator must deploy or delegate validation code and use an ordinary `VERIFY` frame.
...
| `MAX_PENDING_TXS_PER_ACTOR_SLOT` | `1` | Maximum pending transactions that may depend on one exact EIP-8130 actor-validation storage slot |
...
2. the block timestamp as read by an expiry verifier frame or compared with an `EIP8130_ACTOR` expiry,
...
7. for each `EIP8130_ACTOR` signature entry, the exact `(ACCOUNT_CONFIG_ADDRESS, storage_slot)` returned by signature validation, subject to the per-slot pending-transaction limit below.
...
3. `self_verify` and `only_verify` must execute in `VERIFY` mode, target `tx.sender` (either explicitly or via a null target), and produce the approval effect requested by `frame.flags`, either through a successful `APPROVE` call or registered-static evaluation.
    - `self_verify` must produce `APPROVE_EXECUTION_AND_PAYMENT`.
    - `only_verify` must produce `APPROVE_EXECUTION`.
4. `pay` must execute in `VERIFY` mode, have flags set to `APPROVE_PAYMENT`, and produce `APPROVE_PAYMENT` through either a successful `APPROVE` call or registered-static evaluation.
...
9. Every `EIP8130_ACTOR` entry must have empty `msg` and must be the signature at the deterministic index selected by a registered-static `VERIFY` frame in the validation prefix. Reject unused entries, entries consumed only outside the validation prefix, and explicit-message entries.
...
The complete state dependency set of such a validation prefix is: the sender's nonce, code hashes for frames that actually depend on code, the payer's balance (or the canonical paymaster's tracked state), the runtime code at `EXPIRY_VERIFIER` together with the frame's deadline when an expiry verifier frame is present, the exact `(ACCOUNT_CONFIG_ADDRESS, storage_slot)` and `expiry` returned by every `EIP8130_ACTOR` validation, and the current block timestamp. The code hash of a registered-static frame's resolved target is not a validation dependency because that code is never loaded or executed. Nodes SHOULD index pending transactions by this set so that head-of-chain changes are revalidated without re-execution.
...
- a `self_verify`, `only_verify`, or `pay` frame exits without its required approval effect, whether produced by `APPROVE` or registered-static evaluation
...
For EVM-executed `VERIFY` frames, the usual `STATICCALL` restrictions apply except for the protocol-defined effects of `APPROVE`. Registered-static `VERIFY` frames are evaluated directly and produce no EVM trace. For EVM-executed frames in the validation prefix, the following opcodes are banned, with a few caveats:
...
#### EIP-8130 Actor Dependencies

`EIP8130_ACTOR` has no transaction-selected registry and no implementation-configured authenticator allowlist. Consensus fixes the only storage address and the two accepted native authenticator selectors. Consensus validation records one exact storage key for every entry, whether or not registered-static verification later consumes it.
...
Exact dependency indexing alone does not bound fanout: one actor could sign transactions for many different senders and later revoke its single validation slot. A public mempool node therefore MUST keep no more than `MAX_PENDING_TXS_PER_ACTOR_SLOT` transactions that depend on the same `(ACCOUNT_CONFIG_ADDRESS, storage_slot)`. Replacement of a transaction already occupying that slot follows the ordinary replacement rules and atomically transfers the slot reservation. This limit is a propagation rule, not an additional consensus-validity rule.

An actor expiry is also a validation dependency even though it does not require a storage change. If `expiry != 0`, a node MUST evict the transaction once its view of the next inclusion timestamp would make `block.timestamp > expiry`. An expiry of zero is non-expiring.
...
4. The node directly evaluates registered-static frames and otherwise simulates the validation prefix, enforcing the structural and trace rules above. A `pay` frame whose target runtime code exactly matches the canonical paymaster implementation is handled via the canonical paymaster exception and the paymaster-specific rules below. Confirm that every `EIP8130_ACTOR` entry was actually consumed by registered-static verification; otherwise reject.
...
7. If a canonical paymaster instance is used, the node verifies paymaster solvency using the reservation rule above.
8. A node should keep at most one pending frame transaction per sender in the public mempool. A new transaction from the same sender MAY replace the existing one only if it uses the same nonce and satisfies the replacement rules below.
9. If all checks pass, the transaction may be accepted into the public mempool and propagated to peers.
...
When local resource limits are reached, a node should evict in this order: first transactions already invalid against the current head, then transactions with the nearest expiry deadline, then transactions with the lowest effective priority fee. Evicted and replaced transactions must not be re-propagated. Re-admission requires receiving the transaction again. On any eviction or replacement the corresponding payer reservation or exposure and actor-slot reservations are released.

#### Revalidation

When a new canonical block is accepted, the node removes any included frame transactions from the public mempool, updates paymaster and actor-slot reservations accordingly, and identifies the remaining pending transactions whose tracked dependencies were touched by the block. This includes at least transactions for the same sender, transactions whose recorded sender storage slots changed, transactions that reference a canonical paymaster instance whose balance, code, or delayed-withdrawal state changed, transactions whose payer's balance or code changed, transactions whose exact `EIP8130_ACTOR` validation storage slot changed, and transactions whose actor expiry is no longer live. The node then re-simulates the validation prefix of only those affected transactions against the new head and evicts any transaction that no longer satisfies the public mempool rules.
...
The witness portion of signatures with empty `msg` is elided from the canonical signature hash. For `EIP8130_ACTOR`, the 20-byte authenticator selector remains and only its following `data` is elided. This is done for three reasons:
...
### EIP-8130 actor fast path

`EIP8130_ACTOR` lets any account reuse EIP-8130's actor authorization surface as a registered-static root-signature path. Registration is the account's explicit opt-in. When a `VERIFY` frame selects an `EIP8130_ACTOR` signature, clients validate the actor natively, check the configured scope and expiry, and apply the requested approval without loading or executing account code. This applies to code-less, deployed, and EIP-7702-delegated accounts.

The current encoding does not move public keys out of the transaction: P256 still carries `qx` and `qy` in `data`. The state saving is the reuse of the actor's existing authenticator, scope, and expiry configuration rather than a second 8141-specific registry. A future extension may store canonical key material in the registry and reduce the signature witness accordingly.
...
The actor result is cached only for protocol-defined consumers. Avoiding a new EVM-visible actor ABI keeps the fast path from becoming a generic authorization primitive and leaves custom account policy in ordinary `VERIFY` code. `POLICY` actors are always rejected by registered-static verification because the protocol cannot enforce their manager gate without executing account logic.
...
### Registered-static bypasses account validation code

Registering an actor for the `EIP8130_ACTOR` path explicitly authorizes frame transactions to bypass the account's normal `VERIFY` code for the scopes granted to that actor. Accounts MUST treat such an actor as a root protocol credential for those scopes. Wallets SHOULD present registration, scope expansion, and revocation with the same severity as changing an account owner.

The canonical configuration slot is a transaction-validity dependency. Rotating or revoking the actor, changing its authenticator, reducing its scope, or reaching its expiry invalidates affected pending transactions. Public mempool implementations MUST index and revalidate transactions by the exact slot returned during signature validation and enforce `MAX_PENDING_TXS_PER_ACTOR_SLOT`.

The fast path does not execute account code and therefore cannot enforce wallet-specific multisig thresholds, session-key policies, spending limits, guardians, or recovery rules. Those mechanisms MUST use the ordinary EVM `VERIFY` path unless their complete semantics are separately standardized as a bounded protocol path.
...
#### EIP-8130 Actor Configuration is Shared State

An `EIP8130_ACTOR` entry makes transaction validity depend on one slot in the canonical Account Configuration Contract. The exact storage key returned by validation prevents unrelated actors or adjacent configuration state from becoming dependencies. It does not by itself bound fanout: one actor could sign for many senders and revoke that one slot. The public-mempool per-slot limit bounds this invalidation set, while exact-key indexing identifies the transactions that require revalidation. Requiring every propagated entry to have empty `msg` and be consumed by registered-static verification prevents a reusable explicit-digest signature from being attached as an unused dependency to poison the slot reservation.

Actor configuration is read at inclusion time. A configuration change after signing invalidates the transaction, as does passage beyond a nonzero actor expiry. The authenticator selector remains in `compute_sig_hash(tx)` for empty-message signatures, so a witness cannot be transplanted between the K1 and P256 validation rules. Registered-static verification additionally requires an empty message and rejects every `POLICY` actor, preventing an explicit-digest replay or bypass of EIP-8130's manager gate.

Unsupported authenticators execute only inside explicitly budgeted `VERIFY` frames. They are never called while consensus validates the outer signature list.
