...
frames = [[mode, flags, target, limits, value, data, signature_refs], ...]
limits = [execution, state]
signature_refs = [signature_index, ...]
...
- `signature_refs` -- ordered list of zero-based indices into `tx.signatures` that are visible to this frame through `SIGPARAM`. The list may be empty. A signature may be referenced by multiple frames.
...
for i, frame in enumerate(tx.frames):
    ...
    for signature_index in frame.signature_refs:
...
##### Frame-scoped signature references

A signature entry has a global index equal to its zero-based position in `tx.signatures`. A frame does not receive unrestricted access to that global list. Instead, `frame.signature_refs` explicitly selects the entries visible while that frame executes.

`SIGPARAM` interprets its signature index as a frame-local index. For a currently executing frame `frame`, local index `i` resolves to the global entry `tx.signatures[frame.signature_refs[i]]`. An out-of-bounds local index causes an exceptional halt. The same global signature entry may be referenced by more than one frame, but a frame cannot inspect an entry that it does not reference.

Transaction-origination software MUST resolve all component signature requirements into the global `tx.signatures` list and each frame's concrete `signature_refs` before requesting a signature over `compute_sig_hash(tx)`. The global signature ordering, every entry's `scheme`, `signer`, and `msg`, and every frame's `signature_refs` are committed by `compute_sig_hash(tx)`.

After any signature over `compute_sig_hash(tx)` has been requested, the originator MUST NOT add, remove, or reorder signature entries, change their metadata, change any frame's `signature_refs`, or change any other transaction-body field. Raw `signature` bytes of entries with empty `msg` MAY be populated independently after the transaction body is fixed because those bytes are elided by `compute_sig_hash(tx)`. Raw bytes of entries with an explicit `msg` are not elided and MUST already be final before canonical transaction signatures are requested.

For example, a default-code sender using the canonical paymaster can use the following layout:

```text
signatures[0] = [SECP256K1, sender, Bytes(), sender_signature]
signatures[1] = [SECP256K1, paymaster_owner, Bytes(), paymaster_signature]
sender_verify.signature_refs = [0]
pay_frame.signature_refs = [1]
```

Both validation frames address their own signature as local index `0`. The global positions are transaction-construction details, not protocol-assigned roles. Once all other transaction fields are fixed, both raw signatures may be produced independently because both entries have empty `msg`.
...
1. Execute a call with the specified `mode`, `flags`, `target`, `limits`, `value`, `data`, and `signature_refs`. The frame's `signature_refs` become the local signature namespace used by `SIGPARAM` throughout that frame's execution.
...
frame_signature_ref_cost = sum(
    calldata_cost(rlp(signature_index))
...
- If `mode` is `VERIFY`:
  ...
  - If `len(frame.signature_refs) != 1`, revert.
  - Let `sig` be the signature referenced by frame-local signature index `0`.
  - If `sig.scheme != SECP256K1`, `resolved_signer != resolved_target`, or `sig.msg != Bytes()`, revert.
  - Call `APPROVE(allowed_scope)`.
- If `mode` is `SENDER` or `DEFAULT`:
  - Return successfully as if calling empty code.

Each default-code `VERIFY` frame therefore carries exactly one signature reference and accesses it as local index `0`. Sender validation and payment validation may reference different global signature entries without relying on fixed global positions.
...
| 0x0C    | frameIndex   | `len(signature_refs)`                                     |
...
This instruction gives access to signature-scoped metadata through the currently executing frame's `signature_refs`. The raw `signature` bytes of protocol-validated schemes are intentionally not accessible from the EVM. `ARBITRARY` signature bytes are validated during EVM execution and therefore can be accessed with the copy operation.

Let `localSignatureIndex` be the supplied signature index. Before handling any parameter, resolve:

```python
if localSignatureIndex >= len(current_frame.signature_refs):
    exceptional_halt()

globalSignatureIndex = current_frame.signature_refs[localSignatureIndex]
sig = tx.signatures[globalSignatureIndex]
```

For `param` values `0x00` through `0x03`, the gas cost of this operation is `2`. It takes two values from the stack, `localSignatureIndex` on top and `param` second from top, and returns the associated value to the stack.

For `param == 0x04`, this instruction copies bytes from the resolved `ARBITRARY` signature's raw `signature` bytes into memory. Its gas cost is calculated exactly as for `CALLDATACOPY`, including the fixed cost of 3, the per-word copy cost, and the standard EVM memory expansion cost. It takes five values from the stack: `localSignatureIndex` on top, `param` second from top, followed by `memOffset`, `dataOffset`, and `length`. No stack output value is produced.

| `param` | `localSignatureIndex` | Behavior                                     |
|---------|-----------------------|----------------------------------------------|
| 0x00    | localSignatureIndex   | returns `resolved_signer`                    |
| 0x01    | localSignatureIndex   | returns `scheme`                             |
| 0x02    | localSignatureIndex   | returns `msg`                                |
| 0x03    | localSignatureIndex   | returns `len(signature)`                     |
| 0x04    | localSignatureIndex   | copies `ARBITRARY` signature bytes to memory |

Notes:

- Undefined `param` values result in an exceptional halt.
- An out-of-bounds `localSignatureIndex`, or a `signature_refs` entry that is out of bounds for `tx.signatures`, results in an exceptional halt.
- For `ARBITRARY` signature entries, requesting `resolved_signer` results in an exceptional halt.
- For `param == 0x04`, if the resolved signature entry's `scheme` is not `ARBITRARY`, an exceptional halt occurs. The operation semantics match `CALLDATACOPY`, copying `length` bytes from the resolved signature's raw `signature` bytes, starting at `dataOffset`, into memory at `memOffset`. Bytes beyond the end of the signature are copied as zeroes.
...
The canonical paymaster in this draft authorizes with a single protocol-validated `SECP256K1` signature entry and does not support contract-signature schemes. Its `pay` frame MUST have empty `data` and exactly one `signature_refs` entry. The referenced global signature is exposed to the paymaster as frame-local signature index `0` and MUST satisfy all of the following:

- `scheme == SECP256K1`
- `resolved_signer == owner`
- `msg == Bytes()`, so the entry signs `compute_sig_hash(tx)`
- `len(signature) == 65`

The protocol validates the raw signature before any frame executes. The canonical paymaster therefore checks the frame's reference count and the local entry's metadata through `FRAMEPARAM` and `SIGPARAM(0, ...)` before calling `APPROVE(APPROVE_PAYMENT)`. It does not receive a global signature index or raw signature in frame data and does not execute `ecrecover`. Nodes directly evaluating the canonical paymaster MUST apply the same frame-local signature checks. A later specification may define a new canonical implementation version.
...
sequenceDiagram
    ...
    W->>N: Unsigned FrameTx<br/>null limits + signature stubs
    N->>N: Choose provisional ceilings
    N->>N: Execute complete FrameTx
    N->>N: Derive candidate limits
    N->>N: Re-execute with exact candidates
    N->>N: Iterate if required
    N-->>W: intrinsicGas + per-frame limits
```
...
During estimation, protocol-validated `SECP256K1` and `P256` signature entries with empty `msg` MAY set `signature` to `null`. The global signature-array position, metadata, and every frame's concrete `signatureRefs` MUST already be supplied; `null` is only a placeholder for raw signature bytes and does not ask the node to allocate or bind a signature. The node treats such entries as cryptographically valid for simulation and materializes the placeholder as an all-nonzero byte string of the scheme's required signature length: 65 bytes for `SECP256K1` and 128 bytes for `P256`. This materialized length is used for intrinsic calldata gas and for all estimation-visible length queries, including `SIGPARAM(0x03)`. The raw bytes remain inaccessible to EVM code for these protocol-validated schemes. This is a gas-safe upper bound because the final signature has the same length and may contain zero bytes. Signature metadata, including `scheme`, `signer`, and `msg`, is otherwise used exactly as supplied.
...
Signature entries remain reusable transaction-level objects, but each frame explicitly declares which entries it can inspect. This avoids protocol-wide conventions such as assigning the sender to global index `0` and the paymaster to global index `1`. Independent account and paymaster implementations can choose their own global layout while validation code consistently addresses its assigned entries as local indices.

Scoping also reduces accidental authority exposure. A helper or validator reached during one frame cannot inspect an unrelated signature merely because it knows the global array position. Sharing remains possible by placing the same global index in more than one frame's `signature_refs`.

The global list and every frame reference are included in the canonical transaction hash, so composition still has a clear freeze point: all signature metadata and references are fixed before canonical signatures are requested, while empty-`msg` raw signature bytes can be filled afterward.

### Canonical paymaster signature reference

Putting a paymaster's raw signature or global array index in `pay` frame data either forces sequential signing or moves an implicit convention into opaque calldata. The canonical paymaster instead receives one frame-scoped reference and always validates local signature index `0`. The transaction builder may place the underlying signature anywhere in the global list, and the `pay` frame binds the correct entry through `signature_refs`.

Since raw bytes of empty-`msg` entries are elided from `compute_sig_hash(tx)`, the sender and paymaster can sign the same finalized transaction body independently and their signatures can be inserted into the referenced global entries afterward.
...
Frame 0 sets `signature_refs = [0]`, uses local signature index `0` for the sender's empty-`msg` signature, and calls `APPROVE(APPROVE_EXECUTION_AND_PAYMENT)`. Frame 1 sets `signature_refs = []`, executes, and exits normally via `RETURN`.
...
A simple transfer is performed by setting the `SENDER` frame target to the destination account and the frame `value` to the transfer amount. The validation frame uses `signature_refs = [0]`; the transfer frame uses an empty reference list. This requires two frames for mempool compatibility, since the validation phase of the transaction has to be static.
...
The deployment and execution frames use empty signature reference lists, while the validation frame references the sender signature with `signature_refs = [0]`.
...
Frame 0 uses `signature_refs = [0]` and local signature index `0` for the empty-`msg` sender signature before calling `APPROVE(APPROVE_EXECUTION_AND_PAYMENT)`. Frames 1 and 2 use empty reference lists and form an atomic batch: if the swap in frame 2 reverts, the ERC-20 approval from frame 1 is also reverted, preventing the account from being left with a dangling approval.
...
- Frame 0: Uses `signature_refs = [0]`, reads the sender signature as local index `0`, and calls `APPROVE(APPROVE_EXECUTION)`.
- Frame 1: Uses `signature_refs = [1]`, reads the sponsor signature as local index `0`, and checks that the next frame is an ERC-20 send of the right size to the sponsor before calling `APPROVE(APPROVE_PAYMENT)`.
...
| Sender validation frame: signature refs | 2 |
...
| Execution frame: signature refs   | 1     |
| **Total**                         | 144   |
...
| Deployment frame: signature refs | 1 |
...
| Sponsor validation frame: signature refs | 2 |
...
| Send to sponsor frame: signature refs    | 1 |
...
| Sponsor post op frame: signature refs | 1 |
| **Total additional**                 | 154   |
...
A frame-local signature reference scopes visibility but does not itself assign a role or authority. Validation code MUST still verify the resolved entry's expected `scheme`, `resolved_signer`, `msg`, and any relevant length or witness constraints before approving execution or payment. Transaction builders must freeze the global signature ordering, metadata, and every frame's `signature_refs` before requesting canonical transaction signatures.
