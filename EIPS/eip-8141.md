...
category: Core
created: 2026-01-29
requires: 1559, 2718, 3529, 3607, 4844, 7594, 7623, 7702, 7951
---
...
For `CONTRACT_SIGNATURE` entries, the authenticator address is carried inside the `signature` field, so the signature entry shape is unchanged.
...
- `signatures` -- list of typed signature witnesses available to the transaction.
...
- `signer` -- scheme-dependent signer metadata; for `SECP256K1`, `P256`, and `CONTRACT_SIGNATURE`, this is a 20-byte address.
- `msg` -- either empty, indicating the canonical transaction signature hash, or an explicit 32-byte digest.
- `signature` -- raw signature bytes interpreted according to `scheme`; for `CONTRACT_SIGNATURE` entries, `authenticator (20 bytes) || authenticator-specific data`.
...
for sig in tx.signatures:
    ...
    elif sig.scheme == CONTRACT_SIGNATURE:
        assert len(sig.signer) == 0 or len(sig.signer) == 20
        assert len(sig.signature) >= 20
        authenticator = sig.signature[0:20]
        assert authenticator != b"\x00" * 20
...
The `signatures` list contains typed signature witnesses that may be referenced by `VERIFY` frames and by ordinary EVM execution. `SECP256K1` and `P256` entries are validated before any frame is executed. `ARBITRARY` and `CONTRACT_SIGNATURE` entries are only structurally checked at that stage. A `CONTRACT_SIGNATURE` is evaluated lazily if validation code requests its authenticated identity as defined below.

The `signatures` list is optional. Contracts may still ignore it entirely and perform bespoke signature verification inside frame execution. Validation code that needs direct access to raw witness bytes must place them in an `ARBITRARY` signature entry rather than in frame data when the witness signs the canonical transaction signature hash. A `CONTRACT_SIGNATURE` instead keeps its witness opaque to the account and passes it only to the selected authenticator.

The raw `signature` byte strings of `SECP256K1`, `P256`, and `CONTRACT_SIGNATURE` entries are intentionally not introspectable by EVM code to allow future aggregation schemes. For `CONTRACT_SIGNATURE`, the protocol passes the data only to the selected authenticator. Contracts can inspect the authenticator and its returned identity through transaction introspection. The raw `signature` byte strings of `ARBITRARY` entries are introspectable by EVM code through `SIGPARAM`'s copy operation. This means that they cannot be aggregated in the future.
...
| 0x3        | `CONTRACT_SIGNATURE` | `authenticator (20 bytes) || data`, see [Contract Signature Scheme](#contract-signature-scheme) | 100 plus evaluation gas when used |
| 0x4..255   | reserved             | reserved                                       | reserved |

For `SECP256K1`, `P256`, and `CONTRACT_SIGNATURE`, the `signer` is a 20-byte Ethereum address. If absent, `tx.sender` is used (for introspections as well). For `CONTRACT_SIGNATURE`, this is transaction metadata for the consuming account; the protocol does not require the returned identity to equal or otherwise authorize `resolved_signer`.
...
`P256VERIFY(msg, r, s, qx, qy)` uses the verification and failure semantics of [EIP-7951](./eip-7951.md) for the corresponding 160-byte input. The `6700` cost above is the frame-transaction signature cost and does not inherit the EVM precompile gas schedule.

##### Contract Signature Scheme

`CONTRACT_SIGNATURE` is a meta-scheme in which the authenticator contract address selects the concrete signature type and identity namespace. It defines authentication only, not account authorization. The account code that consumes the result decides which `(authenticator, identity)` pairs it trusts and what they may approve.

An entry keeps the ordinary four-element signature shape:

```text
[CONTRACT_SIGNATURE, signer, msg, authenticator || data]
```

`authenticator` MUST be a nonzero 20-byte address. `data` MAY be empty and otherwise has authenticator-defined encoding. The authenticator implements:
...
interface IAuthenticator {
    function authenticate(
        bytes32 hash,
        bytes calldata data
    ) external view returns (bytes32 identity);
}
```

The function selector is `0x30ba1adf`, equal to the first four bytes of `keccak256("authenticate(bytes32,bytes)")`.

The exact calldata encoding is:
...
The offset is measured from the start of the argument block immediately after the four-byte selector.

The effective `hash` is `compute_sig_hash(tx)` when `msg` is empty and is `msg` otherwise. An identity is an opaque value local to its authenticator namespace; only the pair `(authenticator, identity)` identifies a principal. The zero identity is invalid. No authenticator code is executed during outer signature validation. Evaluation occurs only when a `VERIFY` frame requests the identity through `SIGPARAM`, and its work is charged to that frame as specified in [SIGPARAM](#sigparam-instruction-0xb4).
...
The canonical signature hash is computed from a copy of the transaction. For a signature with empty `msg`, witness bytes that would create a circular commitment are elided. A `CONTRACT_SIGNATURE` authenticator address is not witness data: it selects the verification semantics and therefore remains in the hash. Only the bytes after that address are elided.

```python
def compute_sig_hash(tx: FrameTx) -> Hash:
    signing_tx = deep_copy(tx)
    for i, sig in enumerate(tx.signatures):
        if len(sig.msg) == 0:
            if sig.scheme == CONTRACT_SIGNATURE:
                # Keep authenticator, elide only authenticator-specific data.
                signing_tx.signatures[i].signature = sig.signature[0:20]
            else:
                signing_tx.signatures[i].signature = Bytes()
    return keccak(bytes([FRAME_TX_TYPE]) + rlp(signing_tx))
```

`compute_sig_hash` MUST NOT mutate `tx`. Consequently, for empty-message `CONTRACT_SIGNATURE` entries, changing only `data` does not change the hash, while changing `authenticator` does.
...
def validate_signature(sig, tx_sender, sig_hash) -> bool:
    ...
    if sig.scheme == SECP256K1:
        ...
        if recovered == address(0) or resolved_signer != recovered:
            return False
        return True
        ...
        # Contract signatures are evaluated lazily from VERIFY execution.
        # Outer validation checks only their descriptor shape.
        if len(sig.signature) < 20:
            return False
        authenticator = sig.signature[0:20]
        return authenticator != b"\x00" * 20
...
Returning `True` for `CONTRACT_SIGNATURE` means only that the descriptor is well formed. It neither calls the authenticator nor establishes that `resolved_signer` accepts the returned identity.
...
1. In signature-list order, validate or structurally check each `sig` with `validate_signature(sig, tx.sender, sig_hash)`. If it returns `False`, the transaction is invalid. This step does not execute `CONTRACT_SIGNATURE` authenticators.
1. Initialize transaction-scoped variables:
    - `payer = None`
    - `sender_approved = false`
    - `contract_signature_failure = false`

Then for each frame:

1. Execute a call with the specified `mode`, `flags`, `target`, `gas_limit`, `value`, and `data`.
    - On entry to a `VERIFY` frame, initialize an empty journaled authentication cache. A missing signature index has state `UNEVALUATED`. Discard this cache when the top-level frame ends.
...
1. If frame has mode `VERIFY` the following additional requirements are imposed:
    - Execute the frame as a `STATICCALL`, disallowing state manipulation.
        - Only `APPROVE` and the authentication bookkeeping defined for `SIGPARAM(0x06)` can modify transaction-scoped context in `VERIFY`. Authentication bookkeeping cannot modify EVM state or the approval context.
    - If the frame reverts, the transaction is invalid. This would unroll any effects of `APPROVE`.
    - If `contract_signature_failure` is set, the transaction is invalid regardless of whether account code caught the failing subcall. Clients MAY stop execution immediately when the flag is set.
...
After executing all frames, verify that `payer` has been set (i.e. `payer != None`) and that `contract_signature_failure == false`. If either check fails, the whole transaction is invalid. Then, settle fees as defined in [Gas Accounting](#gas-accounting), returning `payer_refund` to the payer.
...
def signature_gas(sig):
    ...
    if sig.scheme == P256:
        return 6700
    if sig.scheme in [ARBITRARY, CONTRACT_SIGNATURE]:
        return 100
    invalid_transaction()
...
signature_verification_cost = sum(
    signature_gas(sig) for sig in tx.signatures
)
...
The `100` gas for `CONTRACT_SIGNATURE` covers only its outer descriptor check. Authenticator evaluation is charged exclusively to the `VERIFY` frame that requests the identity. It is therefore already included in that frame's gross gas used and MUST NOT also be added to `signature_verification_cost`.

Each frame has its own `gas_limit` allocation and the receipt reports the frame's gross gas used. These values do not generally sum to `gas_used`: the intrinsic, per-frame, calldata, and eager signature verification costs are charged outside frame gas limits, and the storage refund and calldata floor apply at the transaction level.
...
- If `mode` is `VERIFY`:
  ...
  - If there is not a `SECP256K1` signature `sig` at index `sig_index` such that `resolved_signer == resolved_target` and `sig.msg == Bytes()`, revert. The default code does not scan other indices.
  - Call `APPROVE(allowed_scope)`.
- If `mode` is `SENDER` or `DEFAULT`:
  - Return successfully as if calling empty code.

Default code rejects `CONTRACT_SIGNATURE` because a transaction-selected authenticator is not an authorization policy. Accounts that use contract-defined signature types must deploy or delegate validation code that checks both the authenticator and returned identity before calling `APPROVE`.
...
- If execution is within a protocol-initiated authenticator call tree, an exceptional halt occurs. This is an authentication failure and cannot modify the approval context.
...
This instruction gives access to signature-scoped metadata. The raw `signature` bytes of `SECP256K1`, `P256`, and `CONTRACT_SIGNATURE` entries are intentionally not accessible from the EVM. `ARBITRARY` signature bytes are validated during EVM execution and therefore can be accessed with the copy operation.

For `param` values `0x00` through `0x03` and `param == 0x05`, the gas cost of this operation is `2`. It takes two values from the stack, `signatureIndex` on top and `param` second from top, and returns the associated value to the stack. Reading `param == 0x05` never executes authenticator code.
...
For `param == 0x06`, the instruction takes the same two stack inputs as the metadata operations and produces one `identity` output. Its base cost is `2`. A cached access has no additional cost. A first access adds the dynamic call charges defined below.
...
| 0x05    | signatureIndex   | returns the `CONTRACT_SIGNATURE` authenticator    |
| 0x06    | signatureIndex   | returns the authenticated `identity`, evaluating it on first access |
...
For `param == 0x05`, if the referenced entry's scheme is not `CONTRACT_SIGNATURE`, an exceptional halt occurs. The first 20 bytes of `signature` are returned as a zero-extended address. This is metadata only: neither the authenticator account nor its code is accessed or warmed.

For `param == 0x06`, ordinary stack, index, scheme, top-level-frame-mode, and base-gas checks occur before authentication evaluation. Stack underflow, an out-of-bounds index, a scheme other than `CONTRACT_SIGNATURE`, a top-level frame mode other than `VERIFY`, or inability to pay the `2` gas base cost causes an ordinary exceptional halt without setting `contract_signature_failure`.

After those checks and the base charge, the current top-level `VERIFY` frame's authentication cache has three states for each signature index: `UNEVALUATED`, `EVALUATING`, and `EVALUATED(identity)`. An `EVALUATED` access returns the cached identity with no additional charge. An `EVALUATING` access is a recursive authentication failure. An `UNEVALUATED` access performs the following operation:

1. Set the entry to `EVALUATING` in the current EVM journal.
2. Let `hash` be the already computed transaction `sig_hash` if `msg` is empty, otherwise `msg`. Let `data` be the bytes after the 20-byte authenticator address.
3. Construct calldata exactly as `abi.encodeCall(IAuthenticator.authenticate, (hash, data))` and perform a protocol-initiated `STATICCALL` to the authenticator.
4. Require the call to succeed with exactly 32 bytes of return data and require the returned `bytes32 identity` to be nonzero.
5. Replace `EVALUATING` with `EVALUATED(identity)` and return `identity`.

The protocol-initiated call has `CALLER = ENTRY_POINT`, `ORIGIN = ENTRY_POINT`, and `CALLVALUE = 0`; `ADDRESS` is the authenticator. Its depth is one greater than the EVM call that executed `SIGPARAM`. It requests all gas remaining after the `2` gas `SIGPARAM` base cost and ordinary `STATICCALL` call-setup charges, and the EIP-150 forwarding cap applies. Ordinary `STATICCALL` call-depth, target-access, cold/warm-account, and child-execution charges apply and are deducted from the current `VERIFY` frame. Supplying the ABI calldata and capturing the 32-byte result have zero memory-expansion and copy cost: caller memory is unchanged and the captured result does not modify the caller's return-data buffer. The authenticator call and all its descendants are static. Executing `APPROVE` anywhere in that call tree exceptionally halts the authenticator call and is an authentication failure under the rule below; it cannot modify the approval context.

The authentication cache is journaled with EVM call scopes: a reverted call discards successful evaluations first created in that scope. It is discarded at the end of the current top-level `VERIFY` frame, so another `VERIFY` frame evaluates against its own execution point. Authenticator account-access effects follow the ordinary transaction access journal. Distinct signature indices have distinct cache entries even if their bytes are identical.

Once an entry has been set to `EVALUATING`, inability to pay any dynamic call charge, any unsuccessful protocol-initiated `STATICCALL`, recursive evaluation, return length other than 32 bytes, or zero identity is an authentication failure. Unsuccessful calls include all EVM exceptional halts, static-context violations, precompile failures, call-depth failures, explicit reverts, and child out-of-gas failures. Authentication failure sets `contract_signature_failure` and exceptionally halts the current EVM call. The flag is not journaled or cleared by EVM reverts. A frame transaction for which this flag was set is invalid even if an ancestor catches the exceptional halt and later calls `APPROVE`.
...
For public mempool accounting, eager signature validation is treated as part of the transaction intrinsic cost and counts against `MAX_VERIFY_GAS`. Any chain-defined native contract-signature evaluation admitted by the rules below also counts against the limit through its containing `VERIFY` frame.
...
9. If validation-prefix execution reaches an uncached `SIGPARAM(0x06)` access for a `CONTRACT_SIGNATURE`, the node MUST reject before any authenticator account, code, or state access; access-list warming; authentication-cache transition; or dynamic call charge. Reading the authenticator with `SIGPARAM(0x05)`, or carrying an entry whose identity is not reached, does not trigger this rule. A future consensus change MAY exempt particular authenticator addresses by defining complete native semantics and a fixed gas cost, or MAY exempt evaluation performed after a non-revertible validation-gas guarantee has taken effect. This EIP defines neither exemption.
...
#### Contract Signature Evaluation

Merely carrying a `CONTRACT_SIGNATURE` creates no account, code, or storage dependency. Outer admission checks its shape without accessing the authenticator, and `SIGPARAM(0x05)` is also a metadata-only operation. Local and private transaction handling may evaluate an identity during `VERIFY`; all of that work, including failure and out-of-gas paths, remains bounded by and charged against the frame's explicit `gas_limit`.

This does not make failed authentication paid work. Under the execution order in this EIP, sender approval precedes payer approval, and a failed `VERIFY` makes the transaction invalid. A later `pay` frame is therefore not a guarantor for earlier validation. Public mempool nodes consequently reject an uncached generic identity access before running it, as required above. Therefore a transaction that actually uses a generic contract identity for validation is local/private/block-only under this EIP's relay rules. This scheme-specific restriction does not remove the bounded simulation work already possible through `ARBITRARY` entries and ordinary calls from validation code. A permissionless generic relay path requires a separate primitive that makes validation gas non-revertibly chargeable even when authentication fails.

A future EIP or chain-specific fork rule may map an authenticator address to native evaluation. Such a rule MUST fix the address, expected code version or code hash, accepted data grammar, byte-level authentication and failure semantics, fixed gas cost, call-context and access-journal effects, behavior when the expected code is absent or mismatched, and activation point as consensus parameters. It MUST preserve the authenticator address returned by `SIGPARAM` and committed by `compute_sig_hash`. It MUST NOT be a client-local allowlist. This EIP defines no native contract authenticator; the existing `SECP256K1` and `P256` schemes remain the fixed-cost native paths.

#### Acceptance Algorithm

1. A transaction is received over the wire and the node decides whether to accept or reject it.
2. The node performs all stateless transaction and signature-shape checks, analyzes the frame structure, and determines the validation prefix. If the prefix is not recognized, reject.
3. The node validates all `SECP256K1` and `P256` signatures and structurally checks all `ARBITRARY` and `CONTRACT_SIGNATURE` entries. If any entry is malformed or an eager signature is invalid, reject. Do not access any contract authenticator.
4. The node simulates the validation prefix and enforces the structural and trace rules above, except that a `pay` frame whose target runtime code exactly matches the canonical paymaster implementation is handled via the canonical paymaster exception and the paymaster-specific rules below. If execution reaches an uncached generic contract-identity request, reject before running the authenticator.
...
The witness portion of signatures with empty `msg` is elided from the canonical signature hash. For `CONTRACT_SIGNATURE`, the 20-byte authenticator address remains and only its following `data` is elided. This is done for three reasons:

1. A signature over `compute_sig_hash(tx)` cannot commit to its own witness bytes, but it can and must commit to metadata that selects the verification rules.
...
### Contract-defined signature types

`CONTRACT_SIGNATURE` separates authentication from authorization. The scheme is a generic wrapper; the authenticator address selects the concrete signature type and converts an authenticator-specific witness into an opaque identity. Deployed or delegated account code then decides whether that `(authenticator, identity)` pair is authorized. This borrows the useful signature-extensibility property of [EIP-8130](./eip-8130.md) without importing its Account Configuration Contract, actor storage, scope, expiry, or default-account policy.

The authenticator address remains in the canonical signature hash because changing it changes the meaning of `data`. The witness itself is elided for an empty `msg` to avoid a circular commitment. Account code can inspect the address before requesting the identity, so an untrusted or unexpectedly expensive authenticator need never execute.

Lazy evaluation keeps arbitrary EVM work out of outer signature validation and charges it to an explicitly bounded `VERIFY` frame. It is not a guarantor: a failed validation still cannot be charged under this EIP's payer ordering. The public relay restriction reflects that distinction rather than treating a gas cap as payment. A later guarantor primitive can make generic evaluation relayable without changing the signature encoding.

Authenticator-address typing also gives chains a stable native-repricing hook. A widely deployed verifier can be replaced by a consensus-native implementation with the same authentication result and failure rules but a fixed lower cost. Wallets continue to submit the same authenticator address and data, and account policy continues to observe the same `(authenticator, identity)` pair. New native mappings require an explicit fork rule; they are not node policy.
...
#### Contract Signature Trust and Validation Cost

The authenticator in a `CONTRACT_SIGNATURE` is selected by the transaction and is not itself a trust anchor. Account code must check the authenticator address before requesting its identity, then authorize the returned identity in that authenticator's namespace. Comparing an identity to an address, or accepting a nonzero identity from any authenticator, is insufficient. This is why default code never accepts this scheme.

For an empty `msg`, authenticator-specific `data` is not part of `compute_sig_hash(tx)`. An authenticator should enforce a canonical witness encoding and should avoid accepting equivalent witnesses with materially different execution cost. The frame's signed `gas_limit` bounds the maximum work, but a more expensive replacement witness can still turn a valid transaction into an out-of-gas authentication failure. The authenticator address remains committed, so data cannot be moved to a different verification implementation without changing the signed hash.

Lazy evaluation prevents arbitrary contract execution during outer signature validation, but it does not make failed `VERIFY` work chargeable. The current public mempool therefore stops before generic authenticator execution. Local or private relays that choose to perform it should apply strict resource limits. A future guarantor must create a non-revertible payment obligation before authentication; merely checking a later payer frame is not equivalent.

Native authenticator substitutions are consensus changes. If two clients use different address mappings, code versions, result semantics, or costs, they can disagree on transaction validity and gas. Chains that add a native mapping must pin all of these values and provide differential vectors against the contract implementation.
